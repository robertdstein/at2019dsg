...
def res_sumtime(p, nu, time, F, Ferr):
    ...
    logging.debug(f'delta_rat={delta_fit**2 / delta_fit_counter**2:0.1f}')
...
logging.info('done building interpolator. testing:')
for B in [0.05, 2]:
    for nu in [1.5e9, 17e9]:
        for p in [2.1,3.5]:
            logging.debug(f'(B, nu, p)= {B}, {nu/1e9}, {p}')
            logging.debug('diff alpha (dex): {0:0.3f}'.format(float(interp_alpha([np.log10(B), np.log10(nu), np.log10(p)])- np.log10(sync.alpha_nu(B, nu, p, gamma_min=gamma_min,  gamma_max=gamma_max, eps_e=eps_e)))))
            logging.debug('diff Ptot (dex): {0:0.3f}'.format(float(interp_Ptot([np.log10(B), np.log10(nu), np.log10(p)])- np.log10(sync.Ptot(B, nu, p, gamma_min=gamma_min,  gamma_max=gamma_max, eps_e=eps_e)))))
            logging.debug('diff Snu (dex): {0:0.3f}'.format(float(np.log10(sync99_tab(np.log10(np.array([nu])),np.log10(np.array([B])), 1e16, np.log10(np.array([p]))))) \
